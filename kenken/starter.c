...
typedef struct
{
     ...
     */
    char letter;         // char, indicates that the particular cell is still unknown
    unsigned int number; // indicates that the particular cell with among other cells of
                         // the same char have been filled in
    bool guess;          // bool, if true then a guess (potential answer) exists
    guess g;             // guess, which contains both the char and possible number.
                         // all guesses on the board will have the same char (for the same cage)
                         // There cannot be two guesses with different chars on a board.
} entry;
...
typedef struct
{
     ...
     */
    char letter;         // char
    unsigned int number; // the number that must result after applying the operator
    char symbol;         // any of: + - = * /
} constraint;
...
/*
    ****************************************************************
    ...
    being solved
    You do not need to access any of these functions to complete the
